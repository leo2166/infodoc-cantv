...
export default function ChatPage() {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      setChatHistory((prev) => [...prev, `DeepSeek: ${text}`]);
    } catch (error) {
      console.error("Error sending message:", error);
      setChatHistory((prev) => [...prev, "DeepSeek: Lo siento, algo salió mal."]);
    } finally {
      setIsLoading(false);
      setMessage("");
    }
          ...
          <CardContent className="space-y-4">
            <div className="h-96 overflow-y-auto p-4 border rounded-md">
              {
                chatHistory.map((msg, index) => {
                  ...
                  const content = isUser ? msg.substring(3) : msg.substring(9).replace(/\n/g, "<br />");
            ...
            <div className="flex space-x-2">
              <Input
                ...
                placeholder="Haz tu pregunta a DeepSeek..."
